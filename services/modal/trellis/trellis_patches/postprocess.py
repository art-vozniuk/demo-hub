...
def to_glb(
    ...
    # UV-space rasterization via PyTorch3D (MIT license).
    # Replaces nvdiffrast (NVIDIA non-commercial) with identical math:
    # triangle rasterization + barycentric interpolation in UV space.
    #
    # Convention: UV [0,1] → NDC [-1,1]; NDC y=+1 → image row 0 (top).
    # Both nvdiffrast and PyTorch3D share this convention, so the baked
    # texture layout is pixel-identical.
    from pytorch3d.renderer.mesh.rasterize_meshes import rasterize_meshes
    from pytorch3d.structures import Meshes

    # Express UV coordinates as 3D NDC vertices (z=0, flat projection).
    uv_verts_ndc = torch.cat(
        [out_uvs * 2 - 1, torch.zeros(len(out_uvs), 1, device="cuda")], dim=-1
    )  # (V, 3)  xy ∈ [-1, 1], z = 0

    uv_mesh = Meshes(verts=[uv_verts_ndc], faces=[out_faces])

    # Orthographic rasterization in UV space.
    # perspective_correct=False: UV-space has no perspective distortion.
    # cull_backfaces=False:      UV charts may have any winding orientation.
    ...
    # pix_to_face: (1, H, W, 1) — face index per texel, -1 for background
    # bary_coords: (1, H, W, 1, 3) — barycentric weights for the 3 face vertices

    pix_to_face = pix_to_face[0, :, :, 0]  # (H, W)
    bary = bary_coords[0, :, :, 0, :]       # (H, W, 3)

    # Mask of valid (triangle-covered) texels.
    mask = pix_to_face >= 0

    # Interpolate 3D vertex positions using barycentric coordinates.
    # Clamp background face indices to 0 to keep indexing in-bounds;
    # those texels are excluded by `mask` downstream.
    safe_face_ids = pix_to_face.clamp(min=0)          # (H, W)
    face_vert_ids = out_faces[safe_face_ids]           # (H, W, 3)
    tri_verts = out_vertices[face_vert_ids]            # (H, W, 3, 3)
    pos = (tri_verts * bary.unsqueeze(-1)).sum(dim=2)  # (H, W, 3)
    pos = pos * mask.float().unsqueeze(-1)             # zero background texels
