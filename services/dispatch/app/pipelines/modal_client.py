...
async def _backoff_sleep(attempt: int) -> None:
    ...
    # concurrent bench requests don't ping Modal in lockstep after a blip.
...
async def invoke_flux_mock(payload: dict[str, Any]) -> dict[str, Any]:
    return await _submit_and_poll(
        label="flux_mock",
        submit_url=config.MODAL_FLUX_MOCK_SUBMIT_URL,
        poll_url=config.MODAL_FLUX_MOCK_POLL_URL,
        submit_url_label="MODAL_FLUX_MOCK_SUBMIT_URL",
        poll_url_label="MODAL_FLUX_MOCK_POLL_URL",
        payload=payload,
    )
