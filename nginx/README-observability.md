...
- `Production Inference (FLUX)` — uid `inference-prod` — RED on the
  dispatch pipeline + Modal-gateway RPS/retries + I/O phase + GPU
  pipe + batch size
- `Cold Start Analysis` — uid `cold-start` — cold-start frequency,
  per-phase p95, container uptime as a $/hr equivalent
...
```
htpasswd -B -c /etc/nginx/pushgateway.htpasswd modal
```

The same plaintext password is what each Modal app reads from the
`pushgateway` secret as `PUSHGATEWAY_TOKEN` (the basic-auth user is
fixed to `modal` in the `prometheus_client` handler — see
